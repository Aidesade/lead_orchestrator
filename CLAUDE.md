...
| `outreach.py` | **главный вход ВЕТКИ** (asyncio): девять стадий рассылки, прекондишены 4 и 5, `--check` = стадия 8, `stage_crm` = стадия 9 (`--no-crm` выключает). Дефолт — черновики, `--send` — реальная отправка |
