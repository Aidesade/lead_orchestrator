...
| `crm_staff_sync.py` | **ССЧ для всех лидов CRM**: свод из партий JSON → кэш `<ORQ_DATA_ROOT>/orq_cache/staff_index.json` → RusProfile по ИНН (`card_by_inn`, при пустом `sshr` — карточка); вердикт `ok/below/unknown/not_found` по `staff_verdict`; `--push` проставляет ССЧ, `--purge` удаляет `below`, `--purge-unknown` — и без показателя за 2025. Без флагов ничего не меняет, пишет `staff_report.json` рядом с партиями |
