...
| 6 | Адрес ЛПР (гипотезы БЕЗ пробивки ящиков) | `email_guess.guess_for_company` + `outreach.pick_recipient` |
...
| 9 | Выгрузка лида в CRM ЦИТ РТ | `crm_push.py` → `POST <CRM_URL>/api/leads/ingest`; вызов в `outreach.stage_crm` |

Нумерация историческая: **стадия 4 (пробивка ящиков) удалена 2026-08-18** — от проверки
существования ящиков (верификатор ЦИТ РТ `EMAIL_VERIFIER_URL` + облачный добор MyEmailVerifier)
отказались. `email_verify.py`/`verify_xlsx.py` остались standalone-утилитами, outreach их не зовёт;
SMTP-проба в стадии 6 выключена кодом (`smtp=False`), гард держит `test_outreach.check_no_verifier_stage`.
...
- **Стадия 5 — прекондишен до первой компании**, а не проверка по ходу: пайплайн, который
  отработал сбор и подбор адресов и только потом упёрся в недоступный ящик отправителя,
  потратил лимиты RusProfile впустую.
- **Стадия 9 — мягкая и только после отправки.** Она единственная идёт ПОСЛЕ необратимого
  действия, поэтому не имеет права ни бросить исключение, ни изменить статус письма: сбой CRM
  пишется в `rec["crm"]` и в лог, прогон продолжается. Ключи не заданы — стадия молча выключена
  (пустой секрет не значит «слать куда попало»). Её результат живёт **рядом** с фактом отправки,
  а не в `STAGES`: добавь мы девятую стадию в общий список — все ранее отработанные компании
  задним числом стали бы в отчёте «частично».
...
py crm_push.py --leads "D:\лиды\leads_state_owned.json"                  # повторить выгрузку по JSON
py crm_push.py --leads "D:\лиды\leads_state_owned.json" --only-missing   # дозалить недоехавших
# округ целиком: госдоля >=25%, выручка >=1 млрд, 14 субъектов ПФО одним прогоном:
set STATE_LEAD_REGION=Татарстан, Башкортостан, Марий Эл, Мордовия, Удмурт, Чуваш, Пермский, Кировская, Нижегородская, Оренбургская, Пензенская, Самарская, Саратовская, Ульяновская&& set STATE_LEAD_REGION_CODE=16,02,12,13,18,21,59,43,52,56,58,63,64,73&& set STATE_LEAD_MIN_REVENUE=1e9&& set STATE_LEAD_MAX_SECONDS=21600&& py orchestrator.py --state-owned --count 150 --collect-only
py orchestrator.py mining --count 10         # явно 10 ВСЕГО по всем отраслям
# по отраслям в ПФО целиком, по 50 НОВЫХ (не заведённых в CRM) на отрасль, сразу в CRM:
set RUSPROFILE_REGION_CODE=16,02,12,13,18,21,59,43,52,56,58,63,64,73&& py orchestrator.py --industries construction,processing,oilgas --per-industry 50 --region "Татарстан, Башкортостан, Марий Эл, Мордовия, Удмурт, Чуваш, Пермский, Кировская, Нижегородская, Оренбургская, Пензенская, Самарская, Саратовская, Ульяновская" --collect-only --push-crm --out "D:\лиды\leads_pfo_constr_proc_oilgas.json"
py orchestrator.py mining,energy --per-industry 10   # 10 НА КАЖДУЮ отрасль (итог 20)
# добор ОДНОЙ отрасли после недобора по гейту ССЧ: резерв должен покрыть и уже отвергнутых карточкой
# (они не в CRM и снова окажутся в верху выдачи), дефолтных +20% на это не хватает:
set RUSPROFILE_REGION_CODE=16,02,12,13,18,21,59,43,52,56,58,63,64,73&& set LEAD_STAFF_RESERVE=40&& py orchestrator.py --industries processing --per-industry 14 --region "Татарстан, Башкортостан, Марий Эл, Мордовия, Удмурт, Чуваш, Пермский, Кировская, Нижегородская, Оренбургская, Пензенская, Самарская, Саратовская, Ульяновская" --collect-only --push-crm --out "D:\лиды\leads_pfo_processing_top.json"
...
py outreach.py "D:\лиды\leads_mining.json" --send --no-crm   # рассылка без выгрузки в CRM (стадия 9)
py crm_push.py --ping                                  # стадия 9: настроена ли CRM и жива ли она
py crm_push.py --demo                                  # стадия 9: залить тестовый лид (ИНН 0000000000)
...
заведённых в выдаче до карточки — `--per-industry 50` значит 50 НОВЫХ, добавлено 2026-09-04),
...
py test_crm_staff_sync.py        # свод ССЧ для CRM: приоритет источников, кэш, вердикты, что идёт в purge
...
`test_state_owned_collection.py`, `test_state_owned_wiring.py`,
...
- **`rusprofile`** (`source_rusprofile.py` + `rusprofile_playwright.py`) — штатный локальный путь.
  ...
  сортировки открывается ровно по одной карточке для выбранных N компаний. Карта `INDUSTRY` — 22 отрасли.
  ⚠️ `okved_strict` матчит ТОЧНЫЙ `main_okved_id`: «06.10» не ловит «06.10.1», а группа «06.1»
  во всём ПФО даёт одну компанию (замер 2026-09-04). Рабочие отрасли перечисляют коды всех уровней
  (`processing`, `oilgas` — «нефтегаз»: добыча, нефтесервис, НПЗ, трубопроводы, ГРО;
  `manufacturing` — «производство»: все 910 кодов раздела C ниже групп из
  `okved2_codes.OKVED2_SECTION_C`, замер по ПФО 2026-09-07: группы NN.N → 39 компаний,
  классы NN.NN → 575, все уровни → 1281 — больше окна выдачи, виден верх по выручке;
  `energy` — «энергетика»: все 49 кодов класса 35 из `okved2_codes.OKVED2_SECTION_D`,
  добавлено 2026-09-09: ТЭЦ, сети и теплосети сидят на видах 35.11.1/35.12.1/35.30.3);
  секции уровня групп (`mining`, `agriculture`, `trade`…) тем же XHR находят почти ничего.
...
локальному реестру), выручка ≥2 млрд ₽ за 2025 из карточки RusProfile, **среднесписочная
численность ≥50 за 2025** с той же карточки (`LEAD_MIN_STAFF`, дефолт 50, `0` — выключить;
правило одно на оба сбора — `source_rusprofile.staff_verdict`, fail-closed: нет показателя
за 2025 или есть только за более ранний год — отказ; окно штата 240–260 удалено 2026-08-19,
порог «от 50» введён 2026-09-03), прямая+косвенная госдоля **≥25% — порог включительный:
...
выдача фильтруется СЕРВЕРОМ RusProfile по коду субъекта (`STATE_LEAD_REGION_CODE`, дефолт
`16`; payload-ключ `region` со списком строк-кодов — без него в топ-1000 РФ по выручке
Татарстана единицы: боевой замер 24/1000 против 660 по коду), чужой регион из выдачи
отсекается ДО карточки, а ПРИНИМАЕТСЯ только субъект РФ из той же выписки ЕГРЮЛ, что
доказала долю («не извлекли адрес» = отказ, fail-closed; пустой `STATE_LEAD_REGION=`
осознанно выключает фильтр целиком). **Оба ключа принимают СПИСОК через запятую —
так собирается округ, а не субъект** (`STATE_LEAD_REGION=Татарстан, Башкортостан, Самарская`
+ `STATE_LEAD_REGION_CODE=16,02,63`): гейт пропускает совпадение с ЛЮБЫМ из
перечисленных, все коды уходят в серверный фильтр одним запросом. В списке держи
ОСНОВУ названия («Самарская», «Пермский», «Удмурт»), а не полное имя субъекта: матч
подстрочный, а выписка ЕГРЮЛ пишет «ОБЛАСТЬ САМАРСКАЯ» обратным порядком слов.
**Город внутри региона — `--city` / `STATE_LEAD_CITY`** (для отраслевого сбора —
`RUSPROFILE_CITY`, добавлено 2026-09-11): серверный фильтр RusProfile знает только код
субъекта, поэтому населённый пункт режется КЛИЕНТСКИ по юрадресу выдачи и, как регион,
ДО карточки. Матч идёт по населённому пункту (`source_rusprofile.city_matches`), а не
подстрокой по адресу: в боевом замере по Башкортостану из 22 адресов со словом
«октябрьск» шесть оказались улицей Октябрьской Революции в Уфе и Октябрьской улицей в
Салавате, а «Октябрьский р-н» — это не город Октябрьский. Пустой адрес при заданном
городе = отказ (fail-closed): выписка ЕГРЮЛ подтверждает субъект, но не населённый пункт.
Несколько городов — через запятую; `*`/«любой» — явный отключатель для PowerShell.
Гард — `check_city_filter` в `test_state_owned_collection.py` и unit-часть
`test_rusprofile_playwright.py`.

⚠️ Округ целиком в один прогон не помещается: RusProfile отдаёт максимум 20 страниц
по 50 = **1000 компаний на запрос**, а в ПФО компаний ≥1 млрд втрое больше — видно
только верх по выручке. Не хватило до N — добирай субъектами по отдельности, у
каждого своё окно в 1000. ⚠️ Окно считается на ЗАПРОС, а не на регион: advanced-search
принимает и верхнюю границу выручки — ключ `finance_revenue_to` (замер 2026-09-11 по
Башкортостану: `from=3e8` → 1764 компании, `from=3e8&to=1e9` → 1178, `from=1e9` → 586,
1178+586=1764; синонима `finance_revenue_till` НЕТ — он молча игнорируется). Значит регион,
не влезающий в 1000, режется полосами выручки и вычитывается целиком. В коде сбора полосы
пока не реализованы — это ручной приём для разовых разведок. Системные сбои источников —
...
| `crm_push.py` | **стадия 9**: лид + факт письма → `POST <CRM_URL>/api/leads/ingest` (заголовок `X-Ingest-Token`), upsert по ИНН. Здесь же маппинг служебных ключей лида (`_inn`, `_industry`, `_revenue`…) в контракт CRM — наружу они не торчат. Только stdlib `urllib`, как в `dadata_enrich`/`checko_enrich`. Ни одна ошибка не покидает модуль: всегда `(ok, причина)`. Здесь же `push_collected` — выгрузка СЫРОГО сбора ФАЗЫ 1 (`--push-crm`, `py crm_push.py --leads <json>`): пакетами по 100 через `POST /ingest/researched-batch` (раздел «Исследован, письмо не готовилось»), с отсевом заведённых по GET-индексу, вычитанием конфликтных ИНН из 409 и остановкой после серии отказов подряд; `--only-missing` требует GET-индекс и отсеивает по нему уже заведённых. `push_staff` — ССЧ уже заведённым лидам (`PUT /ingest/staff`, статус не трогает), `purge_leads` — удаление по ИНН (`POST /ingest/purge`; что нельзя трогать, решает CRM и возвращает в `skipped` с причиной) |
| `crm_staff_sync.py` | **ССЧ для всех лидов CRM**: свод из партий JSON → кэш `<ORQ_DATA_ROOT>/orq_cache/staff_index.json` → RusProfile по ИНН (`card_by_inn`, при пустом `sshr` — карточка); вердикт `ok/below/unknown/not_found` по `staff_verdict`; `--push` проставляет ССЧ, `--purge` удаляет `below`, `--purge-unknown` — и без показателя за 2025 / не найденных, КРОМЕ тех, у кого по последнему известному году ССЧ ≥ порога (решение 2026-09-03). Без флагов ничего не меняет, пишет `staff_report.json` рядом с партиями |
...
⚠️ **Контакты уже заведённого лида обновляются только `PUT /api/leads/ingest/contacts`**
(`crm_push.push_contacts`): `POST /ingest` вместе с контактами переставляет статус на
«письмо отправлено». PUT ничего не создаёт (неизвестный ИНН → `not_found`), пустые поля не
шлёт вовсе — «не нашли телефон» не должно стирать найденный раньше. Ответ `405` означает
«ручка не выкачена на прод», и `push_contacts` печатает это словами, а не кодом.

**ССЧ в CRM (2026-09-03).** Среднесписочная численность (`staff_count`/`staff_year`,
показатель RusProfile по данным ФНС) едет в `build_payload` вместе с лидом — и в
`POST /ingest`, и в `researched-batch`; старым лидам её дозаполняет `crm_push.push_staff`
через `PUT /ingest/staff` (та же логика, что у контактов: статус не трогается, без
численности год не шлётся). **Единственная удаляющая ручка — `POST /ingest/purge`**
(`crm_push.purge_leads`): CRM сносит только нетронутые лиды лидгена, а лид с ответом,
звонком, отказом, сделкой или заведённый вручную оставляет и возвращает в `skipped` с
причиной — переспрашивать и повторять запрос нельзя, удаление необратимо. Свод и чистка
по порогу «ССЧ ≥50 за 2025» — `crm_staff_sync.py` (см. карту кода); обе ручки появились в
CRM миграцией 0017, до выкатки они отвечают 404 и печатаются как «не выкачена».

⚠️ **Статус лида в CRM выбирает не поле, а ручка.** `POST /api/leads/ingest` жёстко
ставит `email_sent` (или `email_draft` при `sent_draft`) — принять `status` снаружи он не
умеет, и собранный, но не тронутый лид попадал бы в раздел «письмо отправлено». Поэтому
сырой сбор ФАЗЫ 1 идёт `POST /ingest/researched-batch`: это единственная ручка, дающая
`researched` — «квалифицирован и исследован, письмо ещё не готовилось». Ручка **create-only
и атомарная**, отсюда в `push_collected` отсев по GET-индексу до отправки, разбор 409
`duplicate_inn` и детерминированный `run_id` (uuid5 от состава ИНН) — повтор после обрыва
попадает в replay пакета, а не заводит второй. Обратная сторона: перевести УЖЕ заведённый
лид в другой статус ingest-токеном нельзя — только руками в CRM.
...
секрет, что `LEADGEN_INGEST_TOKEN` в `.env` самой CRM), `CRM_PUSH=0` — выключить стадию
не трогая ключи, `CRM_PUSH_TIMEOUT` (20 с). Без первых двух стадия молча выключена:
пустой секрет не должен означать «слать лиды в первый попавшийся адрес». Приёмную
сторону см. в репозитории CRM — `backend/app/api/routes/leads.py`.
...
- 30 запросов/мин, 100 бесплатных кредитов в сутки; вердикты кэшируются в
  ...
  заново. Остаток печатается по `py email_guess.py --mev-credits` (из outreach слой убран
  вместе со стадией 4; живым потребителем остался только standalone `verify_xlsx.py`).
...
**Порог ССЧ (оба сбора RusProfile):** `LEAD_MIN_STAFF` (50; `0` — выключить, мусор — стоп
прогона) — среднесписочная численность не ниже порога и строго за `source_rusprofile.STAFF_YEAR`
(2025). ⚠️ Выдача advanced-search показателя НЕ несёт (замер 2026-09-04: ключей `sshr` в items
нет; гейт по выдаче от 2026-09-03 отсекал ВСЁ). Поэтому обычный сбор шлёт серверу предфильтр
`sshr_from` (без года), берёт из выдачи резерв сверх N на отрасль (`LEAD_STAFF_RESERVE`; дефолт —
20% от N, не меньше 3), а решает `staff_gate` по карточке после `enrich_leads` и режет отрасль
до N по выручке; в госрежиме — по блоку карточки после гейта выручки. ⚠️ Добор после недобора
требует ЯВНОГО резерва: отвергнутые карточкой компании в CRM не попали, при повторе снова
окажутся в верху выдачи и снова отсеются, так что резерв должен покрыть их все плюс новый отсев
...
`STATE_LEAD_REGION_CODE` — клиентский `region_included()` при этом продолжает
решать, так что опечатка в коде чужой регион не впустит),
`RUSPROFILE_CITY` (пусто; населённый пункт юрадреса для обычного сбора — то же, что
`--city`/`STATE_LEAD_CITY` в госрежиме), `RUSPROFILE_TIMEOUT_MS`, `RUSPROFILE_BROWSER`
...
- **Асимметрия регион-фильтра.** ВКЛЮЧАЮЩИЙ `--region "Москва"` захватывает и Московскую область
  ...
  «Московская область» остаётся (`region_excluded()`). Алиасы (`мск`/`спб`/`хмао`…) разворачиваются
  только ЦЕЛЫМ словом: голой подстрокой «мск» сидит внутри «Пер**мск**ий», и до 2026-09-07 токен
  «Пермский» превращался в фильтр по Москве — пермские компании молча выпадали из всех отраслевых
  сборов по ПФО (в госрежиме путь другой, там они были). Гард — в `test_rusprofile_playwright.py`.
