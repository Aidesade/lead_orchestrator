...
- **ФАЗА 1 — сбор.** RusProfile по ОКВЭД + **выручка ≥ порога (строго, дефолт 1 млрд ₽)**;
  в этом флоу (`_collect`) выручка берётся ТОЛЬКО из RusProfile — ГИР БО/Dadata/Checko здесь
  не зовутся → обогащение контактов (платный аккаунт) → отбор → `D:\лиды\leads_<отрасли>.json`
...
- **ФАЗА 2 — ресёрч + материалы.** По каждой компании: настоящий deep-research → **ТРИ файла**
  (2 нейтральных .docx + 1 брендированный one-pager .pdf) → заливка в папку компании на Яндекс Диске.
...
1. **Пред-запуск движка ДО сессии писателя** — `deep_research_engine.deep_research()` на верхнем
   уровне. ⚠️ НЕ вызывать движок как live-тул ВНУТРИ сессии писателя: вложенные SDK-сессии сбивают
   писателя, и он не сохраняет документы. Движок отдаёт готовые находки; его сбой компанию не валит
   (вернётся хотя бы официальная база). Standalone-CRA — осознанное исключение: там `deep_research`
...
2. **Обогащение ЛПР (`person_enrich`)** — тоже детерминированно ДО писателя (НЕ вложенная
   ...
   шаг тихо пропускается, если у лида нет `contact_person` или `_inn`.
...
- **Кэш находок движка** `D:\orq_cache\findings_<ИНН>.md` (TTL `ORQ_FINDINGS_TTL_H`=72 ч):
  ретрай писателя и повторный прогон не гоняют deep-research заново.
...
- Опциональные токены: `DADATA_TOKEN`, `CHECKO_TOKEN` (без них работают только ГИР БО + веб).
...
- Тумблеры движка: `DR_USE_CRAWL4AI=0` (→ HTTP-фолбэк), `DR_USE_LLM=0` (regex-only, бесплатно),
