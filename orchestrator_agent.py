...
def _build_cmd(args):
    ...
    cmd += ["--count", str(int(args.get("count") or 10))]   # безопасный дефолт (прод = 200)
...
@tool(
    "run_full_chain",
    "ПОЛНАЯ ЦЕПОЧКА: собрать B2B-лиды по отраслям (RusProfile, выручка выше порога) и по "
    "КАЖДОЙ собранной компании сделать досье + стратегию коммуникации (.docx) в папках "
    "Яндекс Диска. В боевом режиме дорого (~$1–2 за компанию). Chrome при сборе скрыт "
    "(окно не открывается). Доступные отрасли: " + ", ".join(VALID),
    {
        ...
        "count": int,         # сколько компаний (по умолч. 10; прод-объём = 200)
        "min_revenue": float, # порог выручки в рублях (по умолч. 1e9 = 1 млрд)
        "region": str,        # регион названием/аббревиатурой ("" = вся РФ)
...
SYSTEM_PROMPT = (
    "Ты — оператор полной цепочки лидогенерации. Инструмент run_full_chain делает ВСЁ за "
    "один вызов: собирает компании по отрасли (RusProfile, выручка выше порога) и по каждой "
    "готовит досье + стратегию коммуникации (.docx) в папках Яндекс Диска.\n"
    "Подбери ОДИН ключ-отрасль (или несколько через запятую) под запрос из списка "
    "«ключ — название»: " + INDUSTRY_HINT + ". Регион, если назван, передавай в параметр "
    "region КАК ЕСТЬ — названием/аббревиатурой ('ХМАО', 'Югра', 'Татарстан'), НЕ кодом.\n"
    "ДЕНЬГИ И ОБЪЁМ: боевой ресёрч стоит ~$1–2 за компанию. Если пользователь НЕ назвал "
    "число компаний — бери count=10 (безопасно), НЕ 200. Если назвал — используй его. Перед "
    "боевым (не dry_run) запуском ОДНОЙ строкой назови оценку стоимости (count × ~$1.5) и "
    "сразу запускай — НЕ переспрашивай. Для бесплатной проверки связки предлагай dry_run=true.\n"
    "Chrome при сборе по умолчанию СКРЫТ (headed, но окно за экраном — антибот проходит, окна не "
    "видно). show_browser=true ставь ТОЛЬКО если пользователь просит видеть браузер.\n"
    "ПАРАЛЛЕЛИЗМ: workers держи низким (1–2) — каждый ресёрч это отдельный claude CLI (Node, сотни "
    "МБ); при нехватке RAM они падают (0xC0000409), оркестратор сам снизит до 1. НЕ поднимай workers "
    "без явной просьбы.\n"
    "Если пользователь дал путь к готовому JSON лидов — клади его в leads_json (тогда сбор "
    "пропускается, только ресёрч). После завершения коротко отчитайся по результату."
)
