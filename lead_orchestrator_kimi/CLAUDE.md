...
| `research_enrichment_agent.py` | Пять dependency-aware enrichment-субагентов Фазы 2: `official → (contour + secondary) → roles → contacts`; schema v3, exact-URL/redirect provenance, coverage/outreach contracts, persisted evidence trace, schema/prompt/input/dependency/output hashes и non-sliding TTL. **Мягкая деградация (2026-07-22): падение роли → `roles_failed` + частичное досье `complete=False`, НЕ exit 1** (родитель принимает частичное, писатель добирает из находок движка). Trusted-домены засеяны сайтом+почтой лида из Фазы 1 | ✅ офлайн-контракт + selftest (вкл. `test_partial_degradation`) |
