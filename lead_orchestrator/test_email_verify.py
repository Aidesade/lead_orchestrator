...
def test_smtp_session():
    ...
    try:
        ...
        # Код про политику + текст про адресата. Живой случай (Яндекс 360, 2026-08-08):
        # «550 5.7.1 No such user!» — если верить только коду, честный отказ уходит
        # в «не проверили», и адрес остаётся гипотезой навсегда.
        import email_guess as _EG
        check("«5.7.1 No such user» — это отсутствие ящика, а не политика",
              _EG._classify_rcpt(550, b"5.7.1 No such user! 1786201952-WCNX29") == "no")
        check("«5.7.1 Client host blocked» остаётся политикой",
              _EG._classify_rcpt(550, b"5.7.1 Client host blocked") == "policy")
        check("«5.1.1 user unknown» по-прежнему отсутствие ящика",
              _EG._classify_rcpt(550, b"5.1.1 <a@b.ru> user unknown") == "no")
        check("отказ по репутации без слов об адресате — политика",
              _EG._classify_rcpt(554, b"5.7.1 rejected due to reputation") == "policy")
